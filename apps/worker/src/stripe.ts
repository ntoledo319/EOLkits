...
async function createAuditCheckout(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const session = await stripeRequest(env, '/v1/checkout/sessions', {
      ...
      'mode': 'payment',
      'success_url': 'https://eolkits.com/verify?session_id={CHECKOUT_SESSION_ID}',
      'cancel_url': 'https://eolkits.com/audit',
...
async function createPackCheckout(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const session = await stripeRequest(env, '/v1/checkout/sessions', {
      ...
      'line_items[0][quantity]': '1',
      'mode': 'payment',
      'success_url': 'https://eolkits.com/status?session_id={CHECKOUT_SESSION_ID}',
      'cancel_url': 'https://eolkits.com/pack',
      'customer_email': email,
      'metadata[project]': 'eolkits',
