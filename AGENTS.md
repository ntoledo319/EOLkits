...
- **Blocked-cycle law (overrides the Ship law).** When the *environment* — not your judgment — makes an externally visible change impossible (egress blocked, credential revoked, upstream outage, sandbox denial), the cycle ends with **no commit at all**. Record in `DECISIONS.md`: what was attempted, the exact error, and the condition that would let it retry. Then stop. This is a legitimate cycle outcome, not a failed one.
- **No-cosmetic-ship rule.** A cosmetic change is never a ship. Never bump a date, timestamp, build stamp, or version string, and never regenerate a derived artifact, solely so a cycle has output. If the only available change would not exist except to satisfy the Ship law, the correct action is the Blocked-cycle law above. Two consecutive blocked cycles on the same cause is a Human Queue item, not a third attempt.
