...
def render_table(findings: List[Finding]) -> None:
    ...
    util.info(
        f"Scanned {len(findings)} resource(s). "
...
def to_markdown(findings: List[Finding]) -> str:
    ...
    if not al2:
        ...
        return "\n".join(lines)
    lines += [
        "| Type | Resource | Region | Severity | Action |",
        "|---|---|---|---|---|",
    ]
    for f in al2:
        lines.append(
            f"| {f.resource_type} | `{f.resource_id}` | {f.region} | {f.severity} | {f.recommended_action} |"
        )
    lines += ["", "## Primary source", "https://aws.amazon.com/amazon-linux-2/faqs/"]
    return "\n".join(lines)
...
def run(args: argparse.Namespace) -> int:
    ...
    if not machine:
        util.hdr(
            f"Scanning {'fixture ' + args.fixture if args.fixture else (args.regions or 'us-east-1')}"
        )
