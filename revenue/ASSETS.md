...
| [Browser scanner](https://eolkits.com/scan/) and static guidance | Performs local source checks and presents cited migration guidance. | Deterministic Python-generated site on the existing custom host. The GitHub Pages mirror was disabled 2026-09-20 (now 404); it had been a second live checkout. | Free; no uploaded browser-scan source. |
...
| [Audit v2](../HANDOFF.md) API and report runner | Turns one supported upload into a static evidence PDF with locations, hashes and limits. | FastAPI/WeasyPrint on existing GRACE infrastructure. | $299 report; checkout open and live since 2026-09-20; report generation verified in the live container that day (21,624-byte PDF with evidence hash); the remaining fulfillment outcomes in [HANDOFF](../HANDOFF.md) are still unrecorded. |
