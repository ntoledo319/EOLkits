...
## D36 — recover a merge-dropped deliverable and record the failure mode

Merge commit `a5510969` (2026-08-25, part of the GRACE privacy-containment
release, D31) joined two branches that had diverged since roughly 2026-08-22:
one carrying `b97befa7`'s CSP/analytics-containment work, the other carrying
three cloud cycles' worth of `revenue/` journal updates and, critically,
Batches 3-5 of `launch/distribution/repost-answers.md` (211 lines, three days
of cross-checked, ready-to-post drafted answers to real AWS re:Post threads).
Both branches touched every `revenue/*.md` file and `repost-answers.md` on
the same days, so no clean fast-forward existed. The merge resolved this the
way D11/D20/D27/D31 document for this repository — a two-parent commit whose
tree exactly matches one side, preserving the other side as history only —
which is a sound way to reconcile a *generated, always-reproducible* artifact
(a static site build, a container tag) but silently discards genuinely unique
content when applied to a *hand-drafted, non-reproducible* one. Every
`revenue/*.md` file lost its batch3-5-era entries this way; that loss is
low-cost for a journal a later cycle re-narrates anyway (the post-merge
"Hands-off execution state" / "Hands-off retirement and VS release" sections
in PLAN.md independently re-cover the same real-world facts). It was not
low-cost for `repost-answers.md`: that content cannot be regenerated by
restating known facts, since it takes three cycles of live search plus
cross-checking to produce, and it is the standing highest-priority owner
deliverable this loop is asked to keep stocked.

This cycle (2026-08-26) restored `repost-answers.md` verbatim from the
pre-merge tip `7da75425` and appended Batch 6. It did not attempt to recover
the lost `revenue/*.md` entries from that same chain, since their content is
superseded by the current, coherent post-merge narrative and reconstructing
them would add noise without new information.

Standing rule for future cycles: before any two-parent "tree-matches-one-side"
merge across branches that both touched `repost-answers.md` (or any future
file whose content is hand-authored and non-reproducible, e.g. a drafted
dev.to article body), diff that specific file between the two candidate
parents first. If the discarded side has unique, non-superseded additions to
such a file, carry them forward by hand (as this cycle did) rather than
accepting the tree-match wholesale. This does not apply to generated files
(site builds, lockfiles, compiled packages), where taking one side's tree is
safe and correct.
