...
## September 10 operational correction

The custom host now serves the existing tracker through `/_stats/` on its own
origin. All 65 deployed HTML tags and the deployment injector were repaired;
the strict CSP remains, both static guards pass, and an isolated browser's
automatic tracker POST returned 200. This supersedes the external-script
injection blocker described below. It does not establish Audit v2 capability,
purchase delivery, or revenue. Dependency maintenance remains locally prepared
and publication-blocked; see `EMERGENCYDDOTHISNOW.md`.
