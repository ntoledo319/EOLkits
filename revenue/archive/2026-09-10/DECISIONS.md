...
## September 10 — repair the host, preserve compatibility, disclose push denial

The owner authorized completing the emergency handoff and granted a specific
exception for its outside-workspace script and SSH access. The original script
lacked promised HTML backups and reliable idempotency. Use a reviewed in-jail
replacement with validation, backups, preserved ownership and rollback; serve
the tracker first-party without loosening CSP. Production and browser checks
passed. Keep the broad product PR #63 open as the handoff requests; publish
only the maintenance patch separately before refreshed dependency merges.

Close #32 to preserve VS Code 1.85 support, #37 because typescript-eslint 8.69
rejects TypeScript >=6.1, and #36 because Node 26 API types are unnecessary.
Add targeted compatibility ignores while preserving compatible fixes. Test
Python 3.14 behavior and native libraries before accepting either major image.

Publication routes assessed: the combined integration/push was rejected;
separate local cherry-picks succeeded but direct push was rejected; renewed
explicit owner approval still left push rejected. Existing remote PR merges
cannot introduce the required local security patch, and switching to a Git
object API would bypass the same rejected publication action. The next route
is correcting the execution policy for the reviewed push, then fresh CI and
ordered merges. Exact error: `approval required by policy, but AskForApproval
is set to Never`. Do not retry through another transport or fabricate a green
result. Retain the emergency document and uncommitted history/ledger record.

history:recorded eolkits-2026-09-10-emergency-maintenance
